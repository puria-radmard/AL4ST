...
from active_learning.util_classes import (
    OneDimensionalSequenceTaggingDataset,
    SentenceIndex,
)
...
from active_learning.acquisition import (
    RandomBaselineAcquisition,
    LowestConfidenceAcquisition,
    MaximumEntropyAcquisition,
    BALDAcquisition,
    SimpleAggregation,
    EmbeddingMigrationAcquisition,
)
...
from active_learning.selector import (
    FixedWindowSelector,
    SentenceSelector,
    VariableWindowSelector,
)
...
extra_al_attributes = {"emb_mig": {"embeddings": embedding_form}}
...
def configure_al_agent(args, device, model, train_set, helper):
    ...
    if len(args.window) == 1 and int(args.window[0]) == -1:
        if args.beam_search != 1:
            raise ValueError(
                "Full sentence selection requires a beam search parameter of 1"
            )
        selector = SentenceSelector(
            helper,
            normalisation_index=args.alpha,
            round_size=round_size,
            acquisition=acquisition_agg,
            window_class=SentenceSubsequence,
        )
    elif len(args.window) == 1 and int(args.window[0]) != -1:
        selector = FixedWindowSelector(
            helper,
            window_size=int(args.window[0]),
            beta=args.beta,
            round_size=round_size,
        ...
        selector = VariableWindowSelector(
            helper=helper,
            window_range=[int(a) for a in args.window],
            beta=args.beta,
            round_size=round_size,
            beam_search_parameter=args.beam_search,
            normalisation_index=args.alpha,
...
def active_learning_train(args):
    ...
    indices, (train_set, val_set) = random_split(
    ...
    al_attributes = [
        ALAttribute(
            name=k,
            unit_form=v,
            initialisation=[np.nan * np.ones(v(np.array(d[0]))) for d in train_set],
            cache=True,
        )
    ...
    train_set = OneDimensionalSequenceTaggingDataset(
        ...
        labels=[
            torch.nn.functional.one_hot(torch.tensor(d[-1]), len(tag_set))
            for d in train_set
        ],
    ...
    model, criterion = initialise_model(
        word_embeddings, args, helper, device, model_class=Model, loss_type=ModifiedKL
    )
    ...
    for _ in agent:
        ...
        test_loss, test_precision, test_recall, test_f1 = evaluate(
            model,
            GroupBatchRandomSampler(test_data_groups, args.batch_size, drop_last=False),
            test_set,
            helper,
            criterion,
            device,
