...
def make_root_dir(args, indices):
    ...
    root_dir = os.path.join(".", f"{'-'.join(sys.argv[1:])}--{rn}".replace("/", ""))
    os.mkdir(root_dir)
    ...
    with open(os.path.join(root_dir, "config.txt"), "w") as config_file:
        ...
        config_file.write(
            f"AFTER 17/12/2020 LOGGING CHANGE - ALL UNUSED WINDOWS INCLUDED\n"
        )
        config_file.write(
            f"Proportion of initial labelled sentences: {args.initprop} \n"
        )
        ...
        config_file.write(
            f"Size of windows (-1 means whole sentence): {args.window} \n"
        )
        config_file.write(f"\n")
...
def log_round(
    ...
    with open(
        os.path.join(round_dir, f"sentence_prop-{round_num}.tsv"),
        "wt",
        encoding="utf-8",
    ) as f:
        ...
        for (
            sentence_idx,
            labelled_idx,
        ) in agent.train_set.index.labelled_idx.items():  # not used in final

            num_unlabelled = len(agent.train_set.index.unlabelled_idx[sentence_idx])
            num_temp_labelled = len(
                agent.train_set.index.temp_labelled_idx[sentence_idx]
            )
